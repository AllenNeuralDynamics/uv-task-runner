...
Each script is invoked as [`uv run`](https://docs.astral.sh/uv/reference/cli/#uv-run) `<script>` (see [docs](https://docs.astral.sh/uv/guides/scripts/)), so they can declare their own dependencies and Python version using [PEP 723 inline metadata](https://peps.python.org/pep-0723/). No more shared mega-environments.
